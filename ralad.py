...
class Trainer:
    ...
    def process_batch(self, features, label = None, validation=False):
        outputs = {}
        # (6, 1, 128, 8, 8)
        ...
        if validation:
            return outputs
        losses = self.criterion(self.opt, self.weight, labels, outputs, x_feature, retransform_features, label_features, label_retransform_features)
    ...
    def validation(self, log):
        ...
        for feature, labels in self.val_loader:
            with torch.no_grad():
                ...
                feature = feature.to(device)
                labels = labels.to(device)
                outputs = self.process_batch(feature, labels, validation = True)
            # [6, 256, 256]
            pred = np.squeeze(
                torch.argmax(
