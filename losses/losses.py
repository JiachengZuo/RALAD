...
class compute_losses_fine_turn(nn.Module):
    def __init__(self, device='GPU'):
        super(compute_losses_fine_turn, self).__init__()
        self.device = device
        self.L1Loss = nn.L1Loss()

    def forward(self, opt, weight, labels, outputs, features, retransform_features, label_features, label_retransform_features):
        losses = {}
        losses["topview_loss"] = 0
        losses["transform_topview_loss"] = 0
        losses["transform_loss"] = 0
